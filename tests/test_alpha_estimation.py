...
def estimate_gpd_alpha_mle(data: np.ndarray, 
    ...
    Returns:
        包含alpha估计结果的字典
    """
    def generate_estimated_para_dict_from_positive(results, threshold, excesses):
    ...
    def generate_estimated_para_dict_from_negative(results, threshold, excesses):
        if len(excesses) > 10:
            try:
                shape, loc, scale = genpareto.fit(excesses, floc=0)
                alpha = 1.0 / shape if shape > 0 else np.inf
    ...
    if tail_type in ["upper", "both"]:
        ...
        if len(positive_returns) > 0:
            ...
            generate_estimated_para_dict_from_positive(results, threshold, excesses)
    ...
    if tail_type in ["lower", "both"]:
        ...
        if len(negative_returns) > 0:
            ...
            generate_estimated_para_dict_from_negative(results, threshold, excesses)
